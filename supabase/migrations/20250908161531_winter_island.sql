/*
  # Corrigir Erro de Sintaxe dos ENUMs PostgreSQL

  1. Problema Identificado
    - PostgreSQL não suporta `CREATE TYPE IF NOT EXISTS`
    - Necessário usar abordagem condicional com DO blocks

  2. Soluções Implementadas
    - Criação condicional de ENUMs usando DO blocks
    - Verificação de existência antes da criação
    - Criação segura de tabelas e índices
    - Políticas RLS adequadas

  3. Funcionalidades
    - Sistema completo de pagamentos
    - Controle de acesso baseado em assinaturas
    - Verificação automática de acesso premium
*/

-- Criar ENUMs condicionalmente
DO $$
BEGIN
    ...
    -- Criar enum para status de pedidos se não existir
